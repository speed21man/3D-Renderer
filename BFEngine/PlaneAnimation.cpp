...
CPlaneAnimation::CPlaneAnimation(void)
:m_nAniFrame(0)
...
CPlaneAnimation::CPlaneAnimation(LPCTSTR filename, int nTotalFrame, int nWidthNum, int nHeightNum)
:m_nAniFrame(0)
,m_fAniFrameTime(0.0f)
,m_nAniTWidthCount(0)
,m_nAniTHeightCount(0)
...
bool CPlaneAnimation::Init(void)
{
	this->SetVertexBuffer(2);
	this->SetPlaneTexture(m_strTexFilename);

	float fWidth = m_pPlaneTex->ddsd.Width / ((2.0f * 100.0f) * m_nAniTWidthCount);
	float fHeight = m_pPlaneTex->ddsd.Height / ((2.0f * 100.0f) * m_nAniTHeightCount);

	//m_fAniTLength = 1.0f / (float)nAniFrame;

	this->m_fGapWidth = ((0.5f)/ (fWidth * 2 * 100));
	this->m_fGapHeight = ((0.5f)/ (fHeight * 2 * 100));
...
bool CPlaneAnimation::Frame(void)
{
	m_fAniFrameTime += this->m_fAniFrameRate * 0.001f;
	if(m_nAniFrame <= (int)m_fAniFrameTime) m_fAniFrameTime = 0.0f;
...
bool CPlaneAnimation::Render(void)
{
...
bool CPlaneAnimation::Release(void)
{
	return true;
}